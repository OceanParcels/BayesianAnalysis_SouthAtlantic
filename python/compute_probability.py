...
# We need to compute the posteriors with two average windows: 1234 days and
# 30 days. This is way I wraped everything in a for loop.
for average_window in [1234, 30]:
    print('Average time window:', average_window)
    # Empty dictionaries to store computed probabilities.
    likelihood = {}
    posterior = {}
    counts = {}
    avg_label = ''  # label to be modified if average is True.
    ...
    for loc in sources:
        ...
        time_dimensions.append(time)  # to compute the minimum time between loc
    ...
    if compute_mean:
        str = '../analysis/number-particles.nc'
    ...
    if compute_mean:
        ...
        total_counts, _ = time_averaging_field(total_counts,
                                               window=average_window,
                                               normalized=False)
        time = time//average_window
        avg_label = f'average_{average_window}'  # average window nummer
    else:
    ...
    for t in range(time):
        ...
        for j, loc in enumerate(sources):
            ...
            total[j] = likelihood[loc][t]*priors['prior'][loc]
    ...
    for k, loc in enumerate(sources):
        ...
        likelihood_xr[loc] = (["time", "x", "y"], lklhd)  # homgenizing time
    ...
    attributes = {'description': "Posterior probability.",
    ...
    attributes = {'description': "Likelihood.",
                  'average_window': average_window}
