...
def create_border_current(output_name: str, filenames: list, variables: dict,
                          dimensions: dict, grid: array):
    ...
    # Creating the arrays that we will use for the first round of getting the
    # border currents
    ...
    # Looping through all the cells, checking if they are border currents or no
    for i in progressbar.progressbar(range(0, nx)):
        for j in range(1, ny - 1):
            if coastal[j, i] == 1:
                j_steps, i_steps = np.array(
                    [j - 1, j, j + 1]) % nx, np.array([i - 1, i, i + 1]) % ny
